# Official Accessibility Certification Guide

**For Roma Mart Convenience**

---

## Overview

This guide explains how to obtain official, third-party accessibility certifications and maintain them. These certifications demonstrate your commitment to accessibility to customers, partners, and regulatory bodies.

---

## Types of Certifications Available

### 1. WCAG 2.2 Level AA Self-Certification (What You Have Now)

✅ **Status:** Already Achieved  
**Effort:** Minimal (documentation only)  
**Cost:** Free (internal documentation)

**What it is:**

- Your internal audit and testing showing compliance
- Published accessibility statement on your website
- No third party involved

**How to promote it:**

- Add WCAG 2.2 Level AA badge to your website
- Include in your accessibility statement
- Reference in marketing materials

**Next step:** This is your foundation. Now get third-party validation.

---

### 2. Third-Party WCAG 2.2 Audit & Certification

✅ **Recommended**  
**Effort:** Medium (2-4 weeks, active participation required)  
**Cost:** $1,500-$5,000 USD for a full audit

**What it is:**

- Professional accessibility auditor reviews your site
- Issues formal certification letter
- Provides detailed report
- Valid for 12 months (typically)

**Who provides it:**

- **TPGI (The Paciello Group)** – Industry leader
  - Website: <https://www.tpgi.com/>
  - Service: WCAG 2.2 Accessibility Audit & Certification
  - Turnaround: 2-4 weeks
  - Cost: $2,000-$4,000 for small website

- **Deque Systems (Axe)**
  - Website: <https://www.deque.com/>
  - Service: Accessibility Audit + Fixed
  - Cost: $1,500-$3,000

- **WebAIM**
  - Website: <https://webaim.org/>
  - Service: Web Accessibility Audit
  - Cost: $1,200-$2,500
  - Nonprofit-friendly pricing available

- **Access Innovation Media**
  - Website: <https://www.accessinnovation.com/>
  - Service: WCAG 2.2 AA Audits
  - Cost: $1,500-$3,000

**How to get it:**

1. Contact one of the companies above
2. Request "WCAG 2.2 Level AA Audit with Certification"
3. Provide your website URL
4. They'll conduct automated + manual testing
5. Provide report and certification letter
6. Display certificate on your site

**Benefit:** Third-party validation that you're truly accessible

---

### 3. AODA Compliance Verification (Ontario-Specific)

✅ **Recommended for Ontario**  
**Effort:** Low-Medium  
**Cost:** $500-$1,500

**What it is:**

- Formal confirmation of AODA (Ontario) compliance
- Can satisfy legal requirements if audited

**Who provides it:**

- **Accessibility Standards Canada (ASC)**
  - Website: <https://accessibilitystandards.ca/>
  - Service: AODA Compliance Verification
  - Cost: Varies (often included with general audit)

- **Ontario Ministry of Labour**
  - Can provide guidance on compliance
  - Doesn't issue certificates but recognizes third-party audits

**How to get it:**

1. Mention you're in Ontario and need AODA verification
2. Include in your WCAG audit request above
3. Auditor will specifically verify AODA compliance (WCAG 2.0 AA minimum)

---

### 4. ISO/IEC 40500:2025 Certification

✅ **Optional (Advanced)**  
**Effort:** Medium-High  
**Cost:** $2,000-$5,000 (may be bundled with WCAG audit)

**What it is:**

- International standard for web accessibility
- Aligns with WCAG 2.2
- Formal certification from accredited body

**Who provides it:**

- **ISO Certification Bodies** (country-specific)
  - US: <https://www.nsf.org/> (NSF International)
  - Canada: <https://www.nqa.com/> (National Quality Assurance)
  - EU: Various accredited bodies

**How to get it:**

1. Request ISO/IEC 40500:2025 certification
2. Typically conducted alongside WCAG audit
3. Auditor must be ISO-accredited
4. Results in formal ISO certificate

**Note:** Most WCAG auditors can provide ISO alignment verification

---

### 5. EN 301 549 (EAA) Compliance Verification

✅ **Recommended if serving EU**  
**Effort:** Low-Medium  
**Cost:** Often bundled with WCAG audit ($200-$500 extra)

**What it is:**

- European Accessibility Act compliance verification
- Harmonizes with WCAG standards
- Required if targeting EU markets

**Who provides it:**

- Any WCAG auditor can verify EN 301 549 alignment
- European auditors specializing in EAA

**How to get it:**

1. When requesting WCAG 2.2 audit, ask for "EN 301 549 verification"
2. Minimal additional cost if bundled
3. Receive separate statement for EU compliance

---

## Recommended Certification Path for Roma Mart

### Immediate (Next 2-4 Weeks) - **Tier 1 Certification**

**Investment: $2,000-$4,000**

1. **Choose an auditor** (recommended: TPGI or Deque)
2. **Request:** "WCAG 2.2 Level AA Audit with Certification"
3. **Ask for:**
   - Full accessibility audit report
   - Formal certification letter
   - AODA compliance verification (Ontario-specific)
   - EN 301 549 alignment statement (if international)
4. **Timeline:** 2-4 weeks
5. **Deliverables:**
   - Official audit report
   - Certification certificate
   - Recommendations for improvements
   - Permission to display badge on your site

**Action Steps:**

```
1. Email TPGI: sales@tpgi.com
   Subject: WCAG 2.2 Level AA Audit Request - Roma Mart

2. Provide:
   - Site URL: https://khanoflegend.github.io/romamart.ca/
   - Scope: Entire website
   - Industry: Retail/Convenience Store
   - Preferred timeline: ASAP

3. They'll provide quote and timeline
4. Upon agreement, they conduct audit
5. You receive certificate within 4 weeks
```

---

## What the Auditor Will Test

A professional accessibility audit will verify:

✅ **Technical Compliance**

- HTML/CSS/JavaScript validation
- ARIA implementation
- Semantic markup
- Color contrast
- Keyboard navigation
- Screen reader compatibility

✅ **WCAG 2.2 AA Criteria**

- All 50+ Level A & AA success criteria
- Documented violations (if any) with remediation steps

✅ **Best Practices**

- Performance implications of accessibility features
- Mobile accessibility
- Future WCAG 3.0 readiness

✅ **Business Alignment**

- Legal compliance (AODA, ISO, EN 301 549)
- Industry standards
- Competitor benchmarking

---

## After Certification

### Display Your Certificate

Once you receive certification:

1. **Add Certificate Badge to Your Site**

   ```html
   <!-- In footer or accessibility page -->
   <div className="accessibility-badge">
     <img src="/badges/wcag-2.2-aa.png" alt="WCAG 2.2 Level AA Certified" />
     <p>Certified: November 30, 2025</p>
     <p>Valid through: November 30, 2026</p>
   </div>
   ```

2. **Link to Your Certificate**
   - On your Accessibility page
   - In your footer
   - In marketing materials

3. **Update Your Statement**

   ```markdown
   # Accessibility Statement
   
   **Certified WCAG 2.2 Level AA Compliant**
   Audited by: [Auditor Name]
   Certificate Date: November 30, 2025
   Valid Through: November 30, 2026
   
   [View Full Certification Report](link-to-pdf)
   ```

### Maintain Certification

✅ Keep certification current:

- Annual audits recommended
- Update website/pages before next audit
- Monitor for WCAG 3.0 changes
- Respond quickly to accessibility issues

---

## Cost Comparison: Certification Options

| Option | Cost | Effort | Validity | Benefit |
|--------|------|--------|----------|---------|
| **Self-Certification (Current)** | Free | Low | Ongoing | Internal documentation |
| **WCAG 2.2 AA Audit** | $2,000-$4,000 | Medium | 1 year | Third-party validation |
| **WCAG 2.2 + AODA** | $2,200-$4,200 | Medium | 1 year | Legal compliance proven |
| **WCAG 2.2 + ISO 40500** | $2,500-$5,000 | Medium-High | 1 year | International standard |
| **Full Bundle (WCAG + AODA + ISO + EN 301 549)** | $3,000-$6,000 | Medium-High | 1 year | Comprehensive |

**Recommended for Roma Mart:** WCAG 2.2 AA + AODA ($2,200-$4,200)

---

## Certification Auditors - Contact Info

### 1. TPGI (The Paciello Group) - Recommended

- **Website:** <https://www.tpgi.com/>
- **Email:** <sales@tpgi.com>
- **Phone:** +1 (800) 889-9950
- **Services:** WCAG, AODA, ISO audits
- **Turnaround:** 2-4 weeks

### 2. Deque Systems (Axe)

- **Website:** <https://www.deque.com/>
- **Email:** <sales@deque.com>
- **Phone:** +1 (800) 793-3383
- **Services:** WCAG, Deque Certified audits
- **Turnaround:** 1-2 weeks

### 3. WebAIM

- **Website:** <https://webaim.org/>
- **Email:** <info@webaim.org>
- **Services:** WCAG, nonprofit-friendly pricing
- **Turnaround:** 2-3 weeks

### 4. Access Innovation Media

- **Website:** <https://www.accessinnovation.com/>
- **Services:** WCAG 2.2 AA Audits
- **Turnaround:** 2-4 weeks

---

## Email Template for Audit Request

```
Subject: WCAG 2.2 Level AA Audit Request - Roma Mart Convenience

Dear [Auditor Name],

I am reaching out to request a professional accessibility audit of our website. 
We are a convenience store chain in Sarnia, Ontario, and we are committed to 
ensuring our website is accessible to all users.

**Project Details:**
- Website URL: https://khanoflegend.github.io/romamart.ca/
- Scope: Full website audit
- Industry: Retail/Convenience
- Jurisdiction: Ontario, Canada (AODA compliance required)
- Budget: $3,000-$4,500 CAD
- Timeline: ASAP (we want certification within 4 weeks)

**Requested Certifications:**
- WCAG 2.2 Level AA
- AODA Compliance Verification
- EN 301 549 Alignment (if applicable)

**Current Status:**
- Site is already WCAG 2.2 AA self-compliant
- ESLint jsx-a11y checks in place
- Manual testing completed
- Looking for third-party validation

Could you provide:
1. Quote for the full audit
2. Timeline/availability
3. What documentation I should prepare
4. Certificate format and usage rights

Thank you,
[Your Name]
Roma Mart Convenience
+1 (382) 342-2000
accessibility@romamart.ca
```

---

## Free Interim Steps (While Awaiting Professional Audit)

Until you get a professional certification, you can:

1. **Run Automated Tools** (Free)

   ```bash
   npx axe --url https://khanoflegend.github.io/romamart.ca/
   npx pa11y https://khanoflegend.github.io/romamart.ca/
   ```

2. **Use Free Online Validators**
   - WAVE: <https://wave.webaim.org/>
   - Lighthouse: Chrome DevTools → Lighthouse
   - axe DevTools: <https://www.deque.com/axe/devtools/>

3. **Get Expert Feedback**
   - Contact local disability organizations
   - Ask for user testing with actual screen reader users
   - Many nonprofits offer free accessibility reviews

---

## Timeline & Roadmap for Roma Mart

**November 30, 2025:** ✅ Achieved WCAG 2.2 Level AA self-compliance  
**December 2025:** Send certification audit requests  
**January 2026:** Receive official certification  
**January 2026:** Display certificates on website  
**Ongoing:** Maintain compliance, quarterly internal audits  
**November 2026:** Annual professional audit renewal  

---

## Why Third-Party Certification Matters

✅ **Legal Protection:** Demonstrates good-faith compliance effort  
✅ **Customer Trust:** Shows commitment to accessibility  
✅ **Competitive Advantage:** Fewer competitors are fully certified  
✅ **Employee Pride:** Team knows site meets highest standards  
✅ **Regulatory Confidence:** AODA inspectors see third-party validation  
✅ **Insurance:** Some liability policies recognize certified sites  

---

## Questions & Next Steps

**Ready to get certified?**

1. Choose an auditor from the list above
2. Use the email template to send audit request
3. Budget $2,500-$4,500 CAD
4. Timeline: 2-4 weeks

**Need help preparing?**

- Your current accessibility documentation is ready (ACCESSIBILITY_AUDIT.md, ACCESSIBILITY_COMPLIANCE.md)
- ESLint checks are in place
- Manual testing is documented
- Just need third-party auditor to validate

**Questions?**

- Contact your chosen auditor - they'll walk you through the process
- Most offer free consultations before audit

---

**Document Version:** 1.0  
**Last Updated:** November 30, 2025  
**Next Review:** When certification received
