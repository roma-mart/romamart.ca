...
function checkEnvironment() {
  ...
  try {
    ...
    const readPkgVer = (mod) => {
      try {
        return JSON.parse(fs.readFileSync(path.join(__dirname, `../node_modules/${mod}/package.json`), 'utf8')).version;
      } catch {
        return null;
      }
    };
    ...
    issues[SEVERITY.INFO].push({
      ...
      file: 'env',
      line: 0,
      message: `Runtime: node ${nodeVer}, npm ${npmVer}, npx ${npxVer}`,
      code: '',
      fix: 'Ensure LTS stable and compatible toolchain.',
    });
    if (viteVer)
      issues[SEVERITY.INFO].push({
        category: CHECKS.CODE_QUALITY,
    ...
    if (lucideVer)
      issues[SEVERITY.INFO].push({
        ...
        file: 'env',
        line: 0,
        message: `lucide-react ${lucideVer}`,
        code: '',
        fix: '',
      });
    ...
    if (major >= 24) {
      issues[SEVERITY.MEDIUM].push({
        ...
        file: 'env',
        line: 0,
        message: 'Using Node >=24. Ensure Vite/plugins fully support this runtime.',
        code: `node ${nodeVer}`,
        fix: 'Keep Vite/@vitejs/plugin-react on latest, update transitive deps.',
      });
    }
    ...
    if (esbuildVer) {
      ...
      if (a === 0 && (b < 24 || (b === 24 && c <= 2))) {
        issues[SEVERITY.HIGH].push({
          ...
          line: 0,
          message: 'esbuild vulnerable (<=0.24.2) to dev server request leak',
          code: `esbuild ${esbuildVer}`,
          fix: 'Update to >=0.27.x and ensure override pins new version.',
        });
      }
    }
    ...
    try {
      ...
      if (fs.existsSync(lucideEsm)) {
        ...
        if (buf.includes(0)) {
          issues[SEVERITY.HIGH].push({
            category: CHECKS.PERFORMANCE,
            file: 'node_modules/lucide-react/dist/esm/lucide-react.js',
            line: 0,
            message: 'NUL byte detected in lucide-react ESM bundle (rollup parse error risk).',
            code: 'Unexpected character \0',
            fix: 'Reinstall dependencies; upgrade lucide-react to latest; clear npm cache.',
          });
        }
      }
    } catch {}
    ...
    issues[SEVERITY.INFO].push({
      ...
      file: 'env',
      line: 0,
      message: 'Environment check partial failure',
      code: String(e),
      fix: 'Verify node/npm and installed module versions.',
    });
  }
}
  ...
  if (fs.existsSync(appPath)) {
    ...
    if (defaultExportCount > 1) {
      issues[SEVERITY.CRITICAL].push({
        ...
        file: rel,
        line: 0,
        message: 'Multiple exports with the same name "default" for App',
        code: 'export default App; export default App;',
        fix: 'Keep a single default export.',
      });
    }
    ...
    if (opens !== closes) {
      issues[SEVERITY.CRITICAL].push({
        ...
        file: rel,
        line: 0,
        message: 'Mismatched <LocationProvider> wrapper tags',
        code: `open:${opens} close:${closes}`,
        fix: 'Ensure each opening tag has a matching closing tag, remove stray </div> before closing.',
      });
    }
    ...
    if (/StructuredData[\s\S]*<\/div>[\s\S]*<\/LocationProvider>/.test(tail)) {
      issues[SEVERITY.HIGH].push({
        ...
        file: rel,
        line: 0,
        message: 'Stray </div> before </LocationProvider> at file end',
        code: '</div> before </LocationProvider>',
        fix: 'Remove stray </div> so wrapper closes correctly.',
      });
    }
  }
}
      ...
      for (const match of matches) {
        ...
        const isFalsePositive =
          ...
          (match[0].includes('text-gray-9') &&
            (line.includes('bg-yellow') || line.includes('bg-blue') || line.includes('bg-green')));
      ...
      if (hexColorMatch && !line.includes('COLORS.') && !line.includes('BRAND_COLORS')) {
        ...
        if (hexMatch) {
          ...
          const allowedColors = [
            ...
            '#F59E0B',
            '#FCD34D',
            '#451A03', // Amber (warning)
            '#059669',
            '#ECFDF5',
            '#34D399',
            '#064E3B', // Emerald (success)
            '#FFF',
            '#FFFFFF',
            '#000',
            '#000000', // Black/white (rare but valid)
          ];
...
function checkSEO() {
  ...
  for (const file of files) {
    ...
    // Skip internal compliance pages -- they are not indexed by search engines
    if (
      relativePath.includes('pages/') &&
      relativePath.endsWith('.jsx') &&
      !relativePath.includes('pages/internal/') &&
      !relativePath.includes('pages\\internal\\')
    ) {
    ...
    lines.forEach((line, idx) => {
      ...
      if (hexMatch) {
        for (const hex of hexMatch) {
          ...
          const isBrandColor = Object.values(BRAND.colors)
            .map((c) => c.toUpperCase())
            .includes(normalized);
          ...
          const semanticColors = [
            ...
            '#F87171',
            '#FCA5A5',
            '#450A0A', // Red spectrum (errors)
            '#78350F',
            '#FFFBEB',
            '#F59E0B',
            '#FCD34D',
            '#451A03', // Amber spectrum (warnings)
            '#059669',
            '#ECFDF5',
            '#34D399',
            '#064E3B', // Emerald spectrum (success)
            '#FFF',
            '#0B0B0B',
            '#F8F8F8',
            '#1A1A1A',
            ...
            '#F4F4F4',
            '#5A5A5A',
            '#A8A8A8', // Infrastructure (grays, neutrals)
      ...
      if (/fontFamily\s*:\s*['"][^'"]*['"]/.test(line)) {
        ...
        if (fontMatch) {
          ...
          const isBrandFont =
            ...
            font.includes('var(--font-heading)') ||
            font.includes('var(--font-body)');
          if (!isBrandFont) {
            issues[SEVERITY.MEDIUM].push({
              category: CHECKS.BRAND_CONSISTENCY,
              file: relativePath,
...
function checkToolingPreflight() {
  ...
  if (!fs.existsSync(postcssConfigPath)) {
    issues[SEVERITY.HIGH].push({
      ...
      line: 0,
      message: 'Missing postcss.config.js',
      code: '',
      fix: 'Create postcss.config.js with tailwindcss and autoprefixer.',
    });
  } else {
    try {
      ...
      issues[SEVERITY.CRITICAL].push({
        ...
        file: 'postcss.config.js',
        line: 0,
        message: 'Failed to load PostCSS config',
        code: String(e).slice(0, 200),
        fix: 'Ensure config exports plugins and dependency packages are valid.',
      });
    }
  }
  ...
  if (fs.existsSync(sucrasePkg)) {
    try {
      ...
      issues[SEVERITY.CRITICAL].push({
        category: CHECKS.CODE_QUALITY,
        file: sucrasePkg.replace(__dirname + '\\..\\', ''),
        line: 0,
        message: 'Invalid sucrase/package.json (node_modules corruption)',
        code: String(e).slice(0, 200),
        fix: 'Clean reinstall: remove node_modules and package-lock.json; npm cache clean --force; npm install.',
      });
    }
  }
}
      ...
      files.forEach((f) => {
        ...
        if (c.includes('?.')) {
          issues[SEVERITY.INFO].push({
            ...
            line: 0,
            message: 'Optional chaining present (transpiled by Vite).',
            code: '?.',
            fix: 'None required.',
          });
        }
        if (c.includes('??')) {
          issues[SEVERITY.INFO].push({
            ...
            file: path.relative(process.cwd(), f),
            line: 0,
            message: 'Nullish coalescing present (transpiled by Vite).',
            code: '??',
            fix: 'None required.',
          });
        }
      ...
      files.forEach((f) => {
          ...
          c.split('\n').forEach((ln, i) => {
            if (/RM</.test(ln) && /(rounded|w-\d+|text-\d+|font-bold)/.test(ln)) {
              issues[SEVERITY.MEDIUM].push({
                ...
                file: rel,
                line: i + 1,
                message: 'Manual brand mark detected; use <Logo />.',
                code: ln.trim().slice(0, 80),
                fix: 'Replace with <Logo size={...} scheme=... />',
              });
            }
          });
        }
        if (!allowedRawBrandFiles.some((x) => rel.endsWith(x))) {
          brandHexes.forEach((hex) => {
            if (c.includes(hex))
              issues[SEVERITY.LOW].push({
                category: CHECKS.BRAND_CONSISTENCY,
                file: rel,
                line: 0,
                message: `Raw brand color ${hex} used; prefer CSS vars/tokens.`,
                code: hex,
                fix: 'Use var(--color-primary)/var(--color-accent) or tokens.brandColors.',
              });
          });
        }
        c.split('\n').forEach((ln, i) => {
          if (/onClick=/.test(ln) && /className=/.test(ln)) {
            (ln.match(/(?:w|h)-(\d+)/g) || []).forEach((token) => {
              ...
              if (!isNaN(n) && n < 11) {
                issues[SEVERITY.MEDIUM].push({
                  ...
                  file: rel,
                  line: i + 1,
                  message: `Potential touch target <44px (${token}).`,
                  code: ln.trim().slice(0, 80),
                  fix: 'Increase size or padding to meet recommended minimum.',
                });
              }
            });
          }
        ...
        if (/pattern id=\"brand-pattern\"/.test(c) && !/BrandPattern/.test(c)) {
          issues[SEVERITY.LOW].push({
            ...
            file: rel,
            line: 0,
            message: 'Inline brand pattern SVG detected; use reusable component.',
            code: 'pattern id="brand-pattern"',
            fix: 'Refactor to shared BrandPattern component.',
          });
        }
      });
      if (!logoImported) {
        issues[SEVERITY.HIGH].push({
          ...
          file: 'src/',
          line: 0,
          message: 'Logo component not imported; ensure <Logo /> is used for brand mark.',
          code: 'Logo.jsx',
          fix: 'Import Logo in Navbar/Footer.',
        });
      }
    };
  }
