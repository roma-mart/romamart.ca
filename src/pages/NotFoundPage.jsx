...
const NotFoundPage = () => {
  ...
  const breadcrumbs = buildBreadcrumbArray('Page Not Found', `${companyData.baseUrl}${currentPath}`);

  // Show main nav links (navbar items, excluding Home)
  const navLinks = NAVIGATION_LINKS.filter((link) => link.showIn.navbar && link.href !== '/');
  ...
  return (
    <>
      ...
      {breadcrumbs && <StructuredData type="BreadcrumbList" data={{ breadcrumbs }} />}

      <div
        className="min-h-[60vh] flex flex-col items-center justify-center px-4 py-20 text-center"
        style={{ backgroundColor: 'var(--color-bg)' }}
      >
        <p className="text-8xl md:text-9xl font-bold mb-4" style={{ color: 'var(--color-accent)' }} aria-hidden="true">
          404
        </p>

        <h1 className="text-3xl md:text-4xl mb-4" style={{ color: 'var(--color-heading)' }}>
          Page Not Found
        </h1>

        <p className="font-inter text-lg max-w-md mb-10" style={{ color: 'var(--color-text-muted)' }}>
          Sorry, the page you&apos;re looking for doesn&apos;t exist or has been moved. Try one of the links below.
        </p>

        <div className="mb-10">
          <Button href={BASE_URL} variant="action" size="lg" icon={<Home size={20} />} aria-label="Go back to homepage">
            Back to Home
          </Button>
        </div>
